...
years <- 1990:2016
file_names <- paste0("W:/C6_Berglund/data/rawdata/SCB TASTE/taste_lev_civil_", years, ".sas7bdat")
...
cr_demographic <- read_sas("W:/C6_Berglund/data/rawdata/SCB TASTE/taste_lev_demografi.sas7bdat") %>% 
  rename_all(tolower) %>% 
...
cr_demographic_issue <- read_sas("W:/C6_Berglund/data/rawdata/SCB TASTE/taste_lev_demografi.sas7bdat") %>% 
  rename_all(tolower) %>% 
  group_by(lopnr) %>%
...
cr_birthcountry <-  read_sas("W:/C6_Berglund/data/rawdata/SCB TASTE/FodLand 2024-11-29/taste_lev_fodland_eu27_2020.sas7bdat") %>% 
  rename_all(tolower) %>% 
  select(lopnr, fodgreg5)
  
#-------------------------------------------------------------------------------
#import datafile that contains the size of region (note located in validate subfolder)
...
years <- 1990:2018
file_names <- paste0("W:/C6_Berglund/data/rawdata/SCB TASTE/taste_lev_lisa_", years, ".sas7bdat")
...
# import newer migration data, located in the the sub migrationer_240926 folder
# note there is an old taste_lev_migrationer file in the main data area

cr_migration <-  read_sas("W:/C6_Berglund/data/rawdata/SCB TASTE/Migrationer_240926/taste_lev_migr_240926.sas7bdat") %>% 
  rename_all(tolower) %>% 
  ...
  # one date does not make sense, remove observation
  filter(datum!="19420000") %>%
...
years <- 1990:2022
file_names <- paste0("W:/C6_Berglund/data/rawdata/SCB TASTE/TASTE_Utbildning/taste_lev_lisa_", years, ".sas7bdat")
